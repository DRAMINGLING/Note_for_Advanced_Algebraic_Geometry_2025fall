...
\begin{definition}
    ...
    \begin{itemize}
        ...
        \end{tikzcd}
        $
        \item Functorial in the sense that $\forall K \subset J$ $k$-vector subspace, 
        $
        \begin{tikzcd}[sep = small]
            0 \arrow[r] &
            J \arrow[r] \arrow[d] &
...
\end{example}
I would like to write the last sentence as a lemma.
\begin{lemma}
    Let $\mathscr{A}$ be an abelian category.
    Assume $A \rightarrow B$ is an injection in $\mathscr{A}$.
    Then the following chain map is zero map in $\mathcal{D}(\mathscr{A})$ if and only if it is zero map in $\mathcal{K}(\mathscr{A})$.
    \begin{equation}
        \begin{tikzcd}
            [A \arrow[r, hookrightarrow] \arrow[d, "id_{A}"] &
            B] \arrow[d] \\
            [A \arrow[r] &
            0]
        \end{tikzcd}
        \label{equation:chain_map_induced_by_identity_map}
    \end{equation}
    \label{Lemma 6.6}
\end{lemma}
\begin{proof}
    Suffice to show the $\Rightarrow$ arrow.
    Assume chain map \ref{equation:chain_map_induced_by_identity_map} is zero map in $\mathcal(K)(\mathscr{A})$,
    then there exists quasi-isomorphism $X^{\bullet} \overset{\varphi}(\rightarrow) [A \hookrightarrow B]$ such that their composition is null-homotopic.
    ...
    \begin{equation}
        \begin{tikzcd}
            ...
            X^{0} \arrow[r] \arrow[d, "\varphi_{0}"] \arrow[ddl, "D"] &
            0 \\
            A \arrow[r, hookrightarrow] \arrow[d, "id_{A}"] &
            B \arrow[d] & 
            0 \\
    ...
    Denote $a_{1} = \varphi_{0}(x_{1}) - b$ and $a_{2} = \varphi_{0}(x_{2}) - b$.
    Note that $a_{1} - a_{2} = \varphi_{0}(x_{1} - x_{2}) = \varphi_{0} \circ \partial_{X}(x') = \varphi_{-1}(x)$.
    Thus $a_{1} - g(x_{1}) - (a_{2} - g(x_{2})) = (a_{1} - a_{2}) - g(x_{1} - x_{2}) = (a_{1} - a_{2}) - \varphi_{-1}(x')= 0$ so that $s$ is well-defined.
    And $s(a) = a - 0 = a$.
    Conclude that chain map \ref{equation:chain_map_induced_by_identity_map} is null-homotopic.
\end{proof}
