...
\newcommand{\abeliangroups}{\mathcal{A}\kern -.5pt b}
...
\begin{lemma}
    ...
    \begin{equation}
        \begin{tikzcd}
            {[A} \arrow[r, hookrightarrow] \arrow[d, "\identity_{A}"] &
            {B]} \arrow[d] \\
            {[A} \arrow[r] &
            {0]}
        \end{tikzcd}
...
\begin{proof}
    ...
    \begin{equation}
        \begin{tikzcd}
            ...
            0 \\
            A \arrow[r, hookrightarrow] \arrow[d, "\identity_{A}" swap] &
            B \arrow[r] \arrow[d] & 
            0 \\
            A \arrow[r] &
            0
    ...
    Thus $D(x_{1}) - a_{1} - (D(x_{2}) - a_{2}) = D(x_{1} - x_{2}) - (a_{1} - a_{2}) = \varphi_{-1}(x') - (a_{1} - a_{2}) = 0$ so that $s$ is well-defined.
    And $s(a) = 0 - (-a) = a$.
    Conclude that chain map \ref{equation:chain_map_induced_by_identity_map} is null-homotopic.
\end{proof}
\begin{remark}
    Note that cohomology group homomorphism induced by chain map \ref{equation:chain_map_induced_by_identity_map} is always a zero map,
    this is also an example that nonzero morphism in $\mathcal{K}(\mathscr{A})$ induces trivial cohomology group homomorphism.
\end{remark}
More specially, there is another funny lemma.
\begin{lemma}
...
\begin{proof}
    ...
    Given two different interpretation of same morphism in $\hom_{\mathcal{D}(\mathscr{A})}(B^{\bullet}, A[0])$,
    ...
    Note that $\hom_{\mathcal{K}(\mathscr{A})}(C^{\bullet}, A[0]) = \hom_{\mathcal{C}(\mathscr{A})}(C^{\bullet}, A[0])$,
...
\end{proof}
\begin{remark}
    We can generalize this lemma to a fact that for all object $A\in \mathscr{A}$ and complex $B^{\bullet}\in \mathcal{C}(\mathscr{A})$,
    $\hom_{\mathcal{D}(\mathscr{A})}(B^{\bullet}, A[i]) = \hom_{\mathscr{A}}(\coker \partial_{B^{\bullet}}^{-i - 1} A)$ and $\hom_{\mathcal{D}(\mathscr{A})}(A[i], B^{\bullet}) = \hom_{\mathscr{A}}(A, \ker \partial_{B^{\bullet}}^{-i})$,
    followed by same idea.
\end{remark}
...
\end{lemma}
\begin{proof}
    By Proposition \ref{Proposition 6.2}, this is equivalent to $\hom_{\mathcal{K}(\mathscr{A})}(A^{\bullet}, I^{\bullet}[n]) = \{0\}$ for all $n$.
...
\end{proof}
Hence $\mathcal{K}'$ satisfies $\hom^{\bullet}(A^{\bullet}, \cdot)$ for all $A^{\bullet}$.
And we can define derived functor, denoted by $R\hom(A^{\bullet}, \cdot)$.
\begin{lemma}
    Let $I^{\bullet}\in \mathcal{K}'$.
...
\end{lemma}
\begin{proof}
    Only need to show that for any morphism $A^{\bullet} \rightarrow B^{\rightarrow} \overset{qis}{\leftarrow} I^{\bullet}$,
...
Question if there exists extension $T' \rightarrow X$ making the daigram commutative
\begin{equation}
    \begin{tikzcd}[sep = tiny]
...
\begin{proof}
    ...
    \begin{equation}
        \begin{tikzcd}
            ...
            0 \\
            \cdots \arrow[r] &
            \mathbb{L}_{X}^{-1} \arrow[r] \arrow[d] &
            \mathbb{L}_{X}^{0} \arrow[r] \arrow[d] &
            0 \\
        \end{tikzcd}
    \end{equation}
    ...
    we have that $\partial_{Lf^{\ast}\mathbb{L}_{X}}^{-1} = \partial_{Lf^{\ast}\mathbb{L}^{\ge -1}}^{-1}$.
    While by Proposition \ref{Proposition 6.2} and Lemma \ref{Lemma 6.7}, 
    \begin{equation}
        \ext_{T}^{0}(Lf^{\ast}\mathbb{L}_{X}, \mathcal{J}) = \hom_{\mathcal{D}(T)}(Lf^{\ast}\mathbb{L}_{X}, \mathcal{J}[0]) = \hom_{\mathcal{C}(T)}(Lf^{\ast}\mathbb{L}_{X}, \mathcal{J}[0]) = \hom_{T}(\coker \partial_{Lf^{\ast}\mathbb{L}_{X}}^{-1}, \mathcal{J})
    \end{equation}
    and
    \begin{equation}
        \ext_{T}^{0}(Lf^{\ast}\mathbb{L}_{X}^{\ge -1}, \mathcal{J}) = \hom_{\mathcal{D}(T)}(Lf^{\ast}\mathbb{L}_{X}^{\ge -1}, \mathcal{J}[0]) = \hom_{\mathcal{C}(T)}(Lf^{\ast}\mathbb{L}_{X}^{\ge -1}, \mathcal{J}[0]) = \hom_{T}(\coker \partial_{Lf^{\ast}\mathbb{L}_{X}^{\ge -1}}^{-1}, \mathcal{J})
    \end{equation}
    Hence the second equality holds.
    For the first equality, as $f^{\ast}$ is right exact functor,
    $\coker \partial_{Lf^{\ast}\mathbb{L}_{X}^{\ge -1}}^{-1} = H^{0}(\totalcomplex (\mathcal{F}^{\ge -1, \ast})) = H^{0}(\mathbb{L}_{X}^{\ge -1}) = \Omega_{X}$, done!
\end{proof}
\begin{remark}
