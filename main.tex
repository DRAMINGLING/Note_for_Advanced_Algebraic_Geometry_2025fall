...
\newcommand{\enm}{\operatorname{End}}
...
\begin{proof}
    ...
    \begin{equation}
        \begin{split}
            & \bullet I' \subset A' \text{ ideal} \\
            & \bullet A'/I' \otimes_{k[\varepsilon]} k \overset{\sim}{\longrightarrow} A/I \\
            & \bullet A'/I' \text{ flat over } k[\varepsilon]
        \end{split}
    \end{equation}
    ...
    \begin{equation}
        \begin{split}
            & \bullet I' \subset A' \text{ ideal} \\
            & \bullet A'/I' \otimes_{k[\varepsilon]} k \overset{\sim}{\longrightarrow} A/I \\
            & \bullet A'/I' \text{ flat over } k[t] \text{ under map } t \longmapsto \varepsilon \\
            & \bullet A/I \overset{\cdot \varepsilon}{\longrightarrow} A'/I' \text{ injective}
        \end{split}
    \end{equation}
...
\begin{equation}
    \begin{split}
        & \bullet \varphi_{ij} \circ \varphi_{jk} = \varphi_{ik} \leftrightsquigarrow \theta_{ij} + \theta_{jk} = \theta_{ik} \\
        & \bullet \text{modify } \varphi_{i} = \varphi_{i}' \circ \psi_{i} \leftrightsquigarrow \theta_{ij} = \theta_{ij}' + \rho_{i} - \rho_{j}
    \end{split}
\end{equation}
Conclude that 1st order deformations of $\mathcal{E}$ in $X$ is $H^{1}(X, \sheafend_{\mathcal{O}_{X}}(\mathcal{E}))$.
\begin{remark}
    If generalize vector bundles to coherent sheaves, the answer is $\ext_{X}^{1}(\mathcal{E}, \mathcal{E})$ since each element in $\ext_{X}^{1}(\mathcal{E}, \mathcal{E})$ gives an extension
    \begin{eqnarray}
...
\begin{proof}
    ...
    By diagram chasing, we find that $\varphi$ gives $P$-module structure on $J$ and $\phi$ gives $A$-module structure on $B, J$, which is independent of choosing of $\varphi$.
    \par
    To get a map $A \rightarrow B'$, want $I \rightarrow J$ to be zero.
    So we need to change $\varphi$.
    For two different $\varphi_{1}$ and $\varphi_{2}$, get $\delta := \varphi_{1} - \varphi_{2}$.
    Since $J^{2} = 0$, we have that 
    \begin{equation}
        \begin{split}
        ...
        \end{split}
    \end{equation}
    Thus $\delta$ satisfies Laibniz's Law and hence $\delta\in Der_{P}(P, J)$ is a derivation.
    \par
    As $A$ is smooth over $k$, there is an exact sequence of locally free sheaves 
    \begin{equation}
        0 \longrightarrow I/I^{2} \longrightarrow \Omega_{P/k} \otimes_{k} A \longrightarrow \Omega_{A/k} \longrightarrow 0
    \end{equation}
    Apply $\hom_{A}(\cdot, J)$, get
    \begin{equation}
        \begin{tikzcd}
            \hom_{A}(\Omega_{P/k} \otimes_{k} A, J) \arrow[r, twoheadrightarrow] \arrow[d, equal] &
            \hom_{A}(I/I^{2}, J) \arrow[d, equal] \\
            \hom_{k}(\Omega_{P/k}, J) \arrow[d, equal] &
            \hom_{P}(I, J) \\
            Der_{P}(P, J) &
        \end{tikzcd}
    \end{equation}
...
\begin{equation}
    \begin{tikzcd}
        ...
        B' \arrow[r] \arrow[d, "id" swap] \arrow[d, "id + \delta", bend left]&
...
\begin{example}
    ...
    \end{equation}
    Fix $\sigma: V/W \rightarrow V$ such that $id_{V/W}$.
...
\end{definition}
For how many deformations, as the argument of 1st order deformation, gluing messages corresponds to element in $H^{1}(X, \sheafend_{\mathcal{O}_{X}}(\mathcal{E}) \otimes_{A} J)$.
...
\begin{definition}
    ...
    \begin{equation*}
        \begin{aligned}
            \bullet & ob(A', u) = 0 \text{ iff $\exists$ lifting of } u,
            \begin{tikzcd}
            ...
            \end{tikzcd}
            \\
            \bullet & \text{Functorial in the sense, } \forall K \subset J \text{ $k$-vector subspace}, 
            \begin{tikzcd}[sep = small]
                0 \arrow[r] &
                J \arrow[r] \arrow[d] &
                A' \arrow[r] \arrow[d] &
            ...
            \end{tikzcd}
            \\
            & \varphi(u, A') \mapsto \varphi(u, A'/K) \text{ under } V \otimes J \rightarrow V \otimes J/K
        \end{aligned}
    \end{equation*}
\end{definition}
...
\begin{proof}
    ...
    Note that by definition of obstruction theory, $id_{V} \otimes \alpha: V \otimes I/\mathfrak{m}_{P}I \rightarrow V \otimes k$ sends $\varphi(u, P/\mathfrak{m}_{P}I) + \mathfrak{m}_{P}^{n}$ to $\varphi(u, P/\mathfrak{m}_{P}I + \mathfrak{m}_{P}^{n} + \ker(\alpha))$.
    \par
...
But here $R$ needn't be artinian local ring so that we cannot give the universal family by $id_{R}\in \hom(R, R)$.
...
\begin{equation*}
    \begin{split}
        & \bullet \text{When pro-representable?} \\ 
        & \bullet \text{When algebraic?} \\
        & \bullet \text{Global moduli}
    \end{split}
\end{equation*}
...
\begin{proof}
    ...
    \begin{equation}
        \begin{tikzcd}
            ...
            \mathcal{F} & \\
            & h_{A \times_{k} k[\varepsilon]} \arrow[uu, "f_{1}", shift left, crossing over] \arrow[uu, "f_{2}" swap, shift right, crossing over] \arrow[ur, "\varphi_{1}", shift left] \arrow[ur, "\varphi_{2}" swap, shift right] &&
    ...
    \begin{equation}
        \begin{tikzcd}
            ...
            h_{A'} \arrow[ur] \arrow[rr] \arrow[dr, "\eta"] &&
            \mathcal{F} & \\
            & h_{A' \times_{A} A''} \arrow[uu, "f_{1}", shift left, crossing over] \arrow[uu, "f_{2}" swap, shift right, crossing over] \arrow[ur, "\varphi_{1}", shift left] \arrow[ur, "\varphi_{2}" swap, shift right] &&
...
\begin{example}
    ...
    \begin{equation}
        \begin{split}
            & \bullet X_{0} \text{ is projective} \\
            & \bullet X_{0} \text{ is affine with isolated singularities}
        \end{split}
    \end{equation}
...
\begin{reason}
    ...
    we can prove that for all deformation $X/A$, $\aut_{A}(X/A) = \{id\}$.
...
\begin{example}
    ...
    \begin{equation}
        \begin{tikzcd}[sep = small]
            \mathcal{L}_{1} \otimes_{A} \mathbb{C} \arrow[rr, "\varphi \otimes id"] \arrow[dr, "\theta_{1}" swap] &&
...
\begin{proof}[\textbf{\emph{Idea}}]
    ...
    \begin{equation}
        \begin{tikzcd}
            0 \arrow[r] & 
            \mathfrak{m}^{n}/\mathfrak{m}^{n + 1} \otimes_{R/\mathfrak{m}^{n + 1}} A_{n} \arrow[r, hookrightarrow] \arrow[d, "id \otimes \varphi_{n}"] &
    ...
    \begin{equation}
        \begin{tikzcd}
            \mathfrak{m}^{n}/\mathfrak{m}^{n + 1} \otimes_{R/\mathfrak{m}^{n + 1}} A_{n} \arrow[r, "\sim"] \arrow[d, "id \otimes \varphi_{n}"] &
            \mathfrak{m}^{n} \otimes_{R } A_{0} \arrow[d, "id \otimes \varphi_{0}"] \\
        ...
        \end{tikzcd}
    \end{equation}
    Hence $id \otimes \varphi_{n} = id \otimes \varphi_{0}$ is surjective.
    ...
    \begin{equation}
        \begin{tikzcd}
            && 0 \arrow[d] &
            0 \arrow[d] & \\
            && \mathfrak{m}^{n} \otimes_{R} A_{0} \arrow[r, twoheadrightarrow, "id \otimes \varphi_{0}"] \arrow[d, hookrightarrow] &
    ...
    \begin{equation}
        \begin{tikzcd}
            \mathfrak{m}^{n} \otimes_{R} A_{0} \arrow[r, twoheadrightarrow, "id \otimes \varphi_{0}"] &
...
\begin{definition}
    ...
    \begin{equation}
        \begin{tikzcd}[sep = small]
            X \arrow[rr, "id"] \arrow[dr, "\Delta"] \arrow[dd, "id"] &&
            X \arrow[dd] \\
...
\begin{remark}
    ...
    But taking cohomology would give up a lot of messages, so we prefer to remember the whole complex.
    \par
...
\begin{definition}
    ...
    (1)$Q$ is multiplicative i.e. $Q \circ Q \subseteq Q$ and $id_{C}\in Q$ for all object $C\in \mathscr{C}$. \\
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{tikzcd}
    \end{equation}
    where $r$ and $g$ are natural inclusion.
...
\begin{example}
    ...
    similarly we have a triangle up to quasi-isomorphism
    \begin{equation}
        \begin{tikzcd}
            A[0] \arrow[r, "u"] &
...
\begin{lemma}
    Let $\mathscr{A}$ be an abelian category.
    Then $\mathcal{K}^{+}(\mathscr{A})$ (resp. $\mathcal{K}^{-}(\mathscr{A})$) can be identified with the localization of $\mathcal{C}^{+}(\mathscr{A})$ (resp. $\mathcal{C}^{-}(\mathscr{A})$) by quasi-isomorphisms.
    \label{Lemma 6.6}
\end{lemma}
\begin{reason}
...
\begin{definition}
    Let $\mathscr{A}, \mathscr{B}$ be abelian categories, $\mathcal{F}: \mathscr{A} \rightarrow \mathscr{B}$ an additive functor, $\mathscr{A}' \subseteq \mathscr{A}$ full additive category.
...
\par
For complexes, by same strategy of construction of Cartan-Eilenburg resolution, there exists resolution $\mathcal{I}^{\bullet, \ast}$ for all $A^{\bullet}\in \mathcal{C}^{+}(\mathscr{A})$. 
Hence we can also define hyper cohomology $R^{i}\mathcal{F}(A^{\bullet}) = H^{i}(\totalcomplex (\mathcal{I}^{\bullet, \ast}))$.
