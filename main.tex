...
\begin{theorem}[\textbf{\emph{Grothendieck}}]
    ...
    Then for all object $A\in \mathscr{A}$, there is a third quadrant spectral sequence with
    \begin{equation}
...
we can replace $\mathcal{F}^{\bullet}$ by $\mathcal{A}^{\bullet}$ in exact sequence \ref{equation:exact_sequence_of_bundle_of_1st_jets}.
...
\begin{equation}
    ...
    \end{split}
\end{equation}
Note that $\ext_{X}^{1}(Lf^{\ast}\mathbb{L}_{X}, \mathcal{G})$ is just where obstruction lies, 
